...
export class AuthService {
  ...
  login(email:string,password:string) {
    ...
    return this.http.post<AuthTokens>(`${this.apiUrl}/api/v1/auth/login`, data)
    .pipe(
      tap(response => {
  ...
  emailExists(email:string) {
    ...
    return this.http.post<EmailExistsResponse>(`${this.apiUrl}/api/v1/auth/is-available`, data)
  }

  registerAndLogin(name:string,email:string,password:string) {
    return this.register(name, password, email)
    .pipe(
      tap((res) => console.log('usuario creado',res)),
  ...
  recovery(email : string) {
    ...
    return this.http.post<RecoveryResponse>(`${this.apiUrl}/api/v1/auth/recovery`, data);
  }
  ...
  getProfile() {
    ...
    .pipe(
      tap(user => {
        console.log(user,'es el usuarioooo');
        this.user$.next(user);
      })
    );
  }
  ...
  refreshToken(refreshToken : string) {
    ...
    return this.http.post<AuthTokens>(`${this.apiUrl}/api/v1/auth/refresh-token`, data)
    .pipe(
      tap(response => {
        this.tokenService.saveToken(response.access_token);
        this.tokenService.saveRefreshToken(response.refresh_token);
